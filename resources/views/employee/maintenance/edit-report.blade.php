...
@section('title', 'Bakım Raporu Düzenle')
...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Bakım Raporu Düzenle</h1>
                <p class="text-gray-600 mt-1">{{ $maintenance->building->name }} - {{ $maintenance->maintenanceTypeLabel }}</p>
            </div>
            <div class="flex items-center space-x-3">
                <span class="px-3 py-1 text-xs font-medium rounded-full
                    @if($report->completion_status === 'tamamlandi') bg-green-100 text-green-800
                    @elseif($report->completion_status === 'kismi_tamamlandi') bg-yellow-100 text-yellow-800
                    @else bg-gray-100 text-gray-800 @endif">
                    {{ $report->completionStatusLabel }}
                </span>
    ...
    <!-- Rapor Bilgileri -->
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6">
        <h2 class="text-lg font-semibold text-gray-900 mb-4">Rapor Bilgileri</h2>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div>
                ...
                <p class="text-sm font-semibold text-primary-600">₺{{ number_format($report->total_cost, 2) }}</p>
            </div>
        </div>
    </div>

    <!-- Düzenleme Formu -->
    <form action="{{ route('employee.maintenance.update-report', $maintenance) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        @csrf
        @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="datetime-local" id="start_time" name="start_time" required
                           value="{{ old('start_time', $report->start_time->format('Y-m-d\TH:i')) }}"
                ...
                <div>
                    ...
                    <input type="datetime-local" id="end_time" name="end_time"
                           value="{{ old('end_time', $report->end_time ? $report->end_time->format('Y-m-d\TH:i') : '') }}"
                           class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">
                    @error('end_time')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div>
                ...
                <textarea id="work_description" name="work_description" rows="5" required
                          ...
                          class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">{{ old('work_description', $report->work_description) }}</textarea>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div id="products-container">
                @php
                    $usedProducts = $report->used_products ?? [];
                @endphp

                @forelse($usedProducts as $index => $usedProduct)
                    <div class="product-row grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
                        <div>
                            ...
                            <select name="products[{{ $index }}][product_id]" class="product-select w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                                <option value="">Ürün Seçin</option>
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" data-price="{{ $product->sale_price }}"
                                            {{ $product->id == $usedProduct['product_id'] ? 'selected' : '' }}>
                                        {{ $product->name }} ({{ $product->code }}) - ₺{{ number_format($product->sale_price, 2) }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="number" name="products[{{ $index }}][quantity]" min="1" step="0.01"
                        ...
                        <div>
                            ...
                            <input type="number" name="products[{{ $index }}][unit_price]" step="0.01"
                                   value="{{ $usedProduct['unit_price'] ?? '' }}"
                                   class="unit-price w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                   placeholder="0.00">
                        </div>
                        <div>
                            <button type="button" class="remove-product px-4 py-3 text-red-600 hover:text-red-800 transition-colors" {{ count($usedProducts) <= 1 ? 'style=display:none;' : '' }}>
                ...
                @empty
                    <div class="product-row grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Ürün</label>
                            <select name="products[0][product_id]" class="product-select w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                                <option value="">Ürün Seçin</option>
                                @foreach($products as $product)
                                    <option value="{{ $product->id }}" data-price="{{ $product->sale_price }}">
                                        {{ $product->name }} ({{ $product->code }}) - ₺{{ number_format($product->sale_price, 2) }}
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Miktar</label>
                            <input type="number" name="products[0][quantity]" min="1" step="0.01"
                                   class="quantity-input w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent"
                                   placeholder="1">
                        </div>
                        <div>
            ...
            <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                <div class="flex justify-between items-center">
                    ...
                    <span id="total-cost" class="text-lg font-bold text-primary-600">₺{{ number_format($report->total_cost, 2) }}</span>
                </div>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <textarea id="problems_found" name="problems_found" rows="4"
                              ...
                              class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">{{ old('problems_found', $report->problems_found) }}</textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="recommendations" name="recommendations" rows="4"
                              ...
                              class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">{{ old('recommendations', $report->recommendations) }}</textarea>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div>
                ...
                <select id="completion_status" name="completion_status" required
                    ...
                    <option value="tamamlandi" {{ old('completion_status', $report->completion_status) === 'tamamlandi' ? 'selected' : '' }}>✅ Tamamlandı</option>
                    <option value="kismi_tamamlandi" {{ old('completion_status', $report->completion_status) === 'kismi_tamamlandi' ? 'selected' : '' }}>⏳ Kısmi Tamamlandı</option>
                    <option value="ertelendi" {{ old('completion_status', $report->completion_status) === 'ertelendi' ? 'selected' : '' }}>⏸️ Ertelendi</option>
                </select>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="customer_signature" value="1"
                               {{ old('customer_signature', $report->customer_signature) ? 'checked' : '' }}
                ...
                <div>
                    ...
                    <input type="text" id="customer_name" name="customer_name"
                           value="{{ old('customer_name', $report->customer_name) }}"
            ...
            <div class="mt-4">
                ...
                <textarea id="customer_notes" name="customer_notes" rows="3"
                          ...
                          class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all">{{ old('customer_notes', $report->customer_notes) }}</textarea>
            </div>
        </div>
        ...
        @if($report->photos && count($report->photos) > 0)
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
                <h3 class="text-lg font-semibold text-gray-900 mb-4">Mevcut Fotoğraflar</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    @foreach($report->photos as $photo)
                        <div class="relative group">
                            <img src="{{ asset('storage/' . $photo) }}" alt="Bakım Fotoğrafı"
        ...
        <!-- Yeni Fotoğraflar -->
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Yeni Fotoğraflar Ekle</h3>
            <div>
...
document.addEventListener('DOMContentLoaded', function() {
    let productIndex = {{ count($usedProducts) }};
    ...
    document.getElementById('add-product').addEventListener('click', function() {
        ...
        newRow.querySelectorAll('select, input').forEach(element => {
            if (element.name) {
                element.name = element.name.replace(/\[\d+\]/, '[' + productIndex + ']');
            }
            element.value = '';
    ...
    document.querySelectorAll('.product-row').forEach(setupProductRow);
...
function removePhoto(photoPath) {
    if (confirm('Bu fotoğrafı silmek istediğinizden emin misiniz?')) {
        // AJAX ile fotoğraf silme işlemi burada yapılabilir
        // Şimdilik sadece DOM'dan kaldır
