...
@section('title', 'Yeni Bakım İşi Ekle - Harmanşah Yazılım')

@section('content')
<div class="p-6">
    <div class="flex justify-between items-center mb-8">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Yeni Bakım İşi Ekle</h1>
            <p class="text-gray-600 mt-1">Bakım işi planlayın ve personel atayın</p>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select id="maintenance_type" name="maintenance_type" required
                            class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        <option value="">Seçiniz</option>
                        <option value="rutin_bakim" {{ old('maintenance_type') === 'rutin_bakim' ? 'selected' : '' }}>Rutin Bakım</option>
                        <option value="ariza_onarim" {{ old('maintenance_type') === 'ariza_onarim' ? 'selected' : '' }}>Arıza Onarım</option>
                        <option value="periyodik_kontrol" {{ old('maintenance_type') === 'periyodik_kontrol' ? 'selected' : '' }}>Periyodik Kontrol</option>
                        <option value="modernizasyon" {{ old('maintenance_type') === 'modernizasyon' ? 'selected' : '' }}>Modernizasyon</option>
                    </select>
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="4" required
                              class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                              placeholder="Yapılacak işlerin detaylı açıklaması...">{{ old('description') }}</textarea>
                </div>
            </div>
