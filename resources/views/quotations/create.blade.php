        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <select name="building_id" id="building_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg">
                        <option value="">Bina seçmeden oluştur</option>
                        @foreach($buildings as $building)
                            <option value="{{ $building->id }}" data-name="{{ $building->name }}" @selected((string) old('building_id', $selectedBuilding?->id) === (string) $building->id)>
                                {{ $building->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="bg-white rounded-xl shadow-md border border-gray-100 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="customer_name" id="customer_name" value="{{ old('customer_name', $selectedBuilding?->name) }}" class="w-full px-4 py-2 border border-gray-300 rounded-lg" required>
                </div>
                <div>
...
document.getElementById('building_id')?.addEventListener('change', function () {
    ...
    const buildingName = selectedOption ? selectedOption.dataset.name : '';
    if (nameField && !nameField.value.trim() && buildingName) {
        nameField.value = buildingName;
    }
});
