...
@section('title', 'Hesap Düzenle - Harmanşah Yazılım')
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Hesap Düzenle</h1>
                    <p class="text-gray-600 mt-1">{{ $accountType->name }} hesabını düzenleyin</p>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <form action="{{ route('account-types.update', $accountType) }}" method="POST" class="p-6">
                @csrf
                @method('PUT')
                ...
                <div class="mb-6">
                    ...
                    <select name="type" id="type" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500">
                        ...
                        <option value="kasa" {{ old('type', $accountType->type) == 'kasa' ? 'selected' : '' }}>Kasa</option>
                        <option value="banka" {{ old('type', $accountType->type) == 'banka' ? 'selected' : '' }}>Banka Hesabı</option>
                        <option value="nakit" {{ old('type', $accountType->type) == 'nakit' ? 'selected' : '' }}>Nakit</option>
                        <option value="pos" {{ old('type', $accountType->type) == 'pos' ? 'selected' : '' }}>POS Cihazı</option>
                    </select>
                    @error('type')
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name', $accountType->name) }}" required
                ...
                <div class="mb-6">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" name="initial_balance" id="initial_balance" value="{{ old('initial_balance', $accountType->initial_balance) }}"
                ...
                <!-- Güncel Bakiye (Sadece görüntüleme) -->
                <div class="mb-6">
                    <label class="block text-sm font-medium text-gray-700 mb-2">Güncel Bakiye</label>
                    <div class="relative">
                        <span class="absolute left-3 top-2 text-gray-500">₺</span>
                        <input type="text" value="{{ number_format($accountType->current_balance, 2) }}"
                               class="w-full pl-8 pr-3 py-2 border border-gray-300 rounded-lg bg-gray-50" readonly>
                    </div>
                    <p class="text-sm text-gray-500 mt-1">Bu değer otomatik olarak güncellenir</p>
                </div>
                ...
                <div id="bank-fields" class="hidden space-y-6">
                    <div class="border-t border-gray-200 pt-6">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="bank_name" id="bank_name" value="{{ old('bank_name', $accountType->bank_name) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="branch_name" id="branch_name" value="{{ old('branch_name', $accountType->branch_name) }}"
                                   class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="account_number" id="account_number" value="{{ old('account_number', $accountType->account_number) }}"
                ...
                <div class="mb-6">
                    ...
                    <textarea name="notes" id="notes" rows="3"
                              class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
                              placeholder="Hesap hakkında ek bilgiler...">{{ old('notes', $accountType->notes) }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
